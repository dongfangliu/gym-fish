...
#         del self.renderer
    ...
    def stepOnce(self,raw_action,save_fluid=False, save_objects=True):
        targetAngle = (3.1415/6)*math.sin(6.28*2*self.rigid_data.rigidWorld.time)
        act = (targetAngle-self.free_robot.getJoint("spine").getPositions()[0])/self.dt
...
#         self.dist_potential = self.calc__dist_potential()
        dist_reward = self.wp*np.exp(-3* (self.walk_target_dist**2))
#         +self.wp[1]*float(self.dist_potential - dist_potential_old)/t*t_standard
